...
namespace Vobiz\Conferences;

use Psr\Http\Client\ClientInterface;
use Vobiz\Core\Client\RawClient;
use Vobiz\Conferences\Types\ListConferencesResponse;
...
class ConferencesClient
{
     ...
     */
    public function listConferences(string $authId, ?array $options = null): ?ListConferencesResponse
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    ...
                    path: "api/v1/Account/{$authId}/Conference/",
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListConferencesResponse::fromJson($json);
            }
     ...
     * @throws VobizApiException
     */
    public function deleteAllConferences(string $authId, ?array $options = null): void
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
    ...
    public function getConference(string $authId, string $conferenceName, ?array $options = null): mixed
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Production->value,
                    path: "api/v1/Account/{$authId}/Conference/{$conferenceName}/",
                    method: HttpMethod::GET,
                ),
                $options,
            );
     ...
     * @throws VobizApiException
     */
    public function deleteConference(string $authId, string $conferenceName, ?array $options = null): void
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
