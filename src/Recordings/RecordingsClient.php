...
use Vobiz\Recordings\Types\GetRecordingResponse;
...
class RecordingsClient
{
     ...
     */
    public function listRecordings(string $authId, ListRecordingsRequest $request = new ListRecordingsRequest(), ?array $options = null): ?ListRecordingsResponse
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    ...
                    path: "api/v1/Account/{$authId}/Recording/",
     ...
     * @throws VobizApiException
     */
    public function getRecording(string $authId, string $recordingId, ?array $options = null): ?GetRecordingResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Production->value,
                    path: "api/v1/Account/{$authId}/Recording/{$recordingId}/",
                    method: HttpMethod::GET,
                ),
                $options,
            );
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return GetRecordingResponse::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new VobizException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
     ...
     * @throws VobizApiException
     */
    public function deleteRecording(string $authId, string $recordingId, ?array $options = null): void
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
